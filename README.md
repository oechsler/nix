...
### Displays (`configuration.nix`)

Per-monitor display configuration. Works on both Hyprland and KDE. When `displays.monitors` is empty (the default), the system falls back to `theme.scale` — no migration needed.
...
Applied to both Hyprland and KDE. On KDE, touchpad settings are detected and configured per-device at activation time.
...
### File Manager Bookmarks (`home.nix`)
