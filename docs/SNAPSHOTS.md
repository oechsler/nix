...
| `@var`     | `/var`     | Variable system state                                  | Only without Impermanence                      |
...
is recreated on boot. The Nix store and `/var` are also excluded because they are
managed separately from the standard home and persist snapshots.
