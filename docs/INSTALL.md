...
`/persist` retain their declared state. Selected state under `/var/lib` is bound
into `/persist`; the rest of `/var` remains part of the ephemeral root. Optional
data subvolumes keep large or
