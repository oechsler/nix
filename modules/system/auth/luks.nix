  ...
  yubikey-luks-init = pkgs.writeShellApplication {
    ...
    text = ''
      ...
      if [[ ''${#DEVICES[@]} -eq 0 ]]; then
        echo "Error: No LUKS devices found in NixOS config." >&2
        exit 1
      fi
      get_fido2_slots() {
      ...
      do_enroll() {
        ...
        for dev in "''${DEVICES[@]}"; do
          ...
          while [[ $ATTEMPT -lt 3 ]]; do
            ATTEMPT=$((ATTEMPT + 1))
            if systemd-cryptenroll "$dev" --fido2-device=auto \
              --fido2-with-client-pin=no --unlock-key-file="$PASS_FILE"; then
              DEV_OK=true
              break
            fi
          done
        ...
        if [[ "$ENROLL_OK" != true ]]; then
          echo "Error: Enrollment failed." >&2
          exit 1
        fi
      }
      ...
      for dev in "''${DEVICES[@]}"; do
        ...
        echo "  $(basename "$dev") ($dev) — ''${#SLOTS[@]} FIDO2 key(s) enrolled"
      done
          ...
          d|D)
            if [[ $SLOT_COUNT -eq 1 ]]; then
              ...
              if [[ "$DEL_CHOICE" == "a" || "$DEL_CHOICE" == "A" ]]; then
                ...
                for dev in "''${DEVICES[@]}"; do
                  systemd-cryptenroll "$dev" --wipe-slot="$SLOT_TO_DEL" || true
                done
              else
