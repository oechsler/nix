    ...
    services.hypridle = {
      ...
      settings = {
        general = {
          ...
          after_sleep_cmd = "${pkgs.runtimeShell} -c 'hyprctl reload 2>/dev/null || true; systemctl --user try-restart awww.service xdg-desktop-portal.service xdg-desktop-portal-hyprland.service xdg-desktop-portal-gtk.service 2>/dev/null || true; hyprctl dispatch dpms off 2>/dev/null || true; sleep 0.5; for i in 1 2 3; do hyprctl dispatch dpms on 2>/dev/null && break; sleep 0.5; done'";
        };
